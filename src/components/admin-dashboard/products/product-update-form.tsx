...
import { CREATE_PRODUCT, UPDATE_PRODUCT } from '../../../services';
import { ADD_TO_PRODUCTS, UPDATE_PRODUCT_STATE } from '../../../store/products';
...
const ProductUpdateForm = ({categories, product}: Props) => {
    const dispatch = useDispatch();
    const [loading, setLoading] = useState<boolean>(false);

    const [productImage, setProductImage] = useState<{value: string, error: boolean } | any>({value: '', error: false});
    const [name, setName] = useState<{value: string, error: boolean } | any>({value: '', error: false});
    const [sku, setSku] = useState<{value: string, error: boolean } | any>({value: '', error: false});
    const [make, setMake] = useState<{value: string, error: boolean } | any>({value: '', error: false});
    const [description, setDescription] = useState<{value: string, error: boolean } | any>({value: '', error: false});
    const [price, setPrice] = useState<{value: number, error: boolean } | any>({value: 0, error: false});
    const [discount, setDiscount] = useState<{value: number, error: boolean } | any>({value: 0, error: false});
    const [quantity, setQuantity] = useState<{value: number, error: boolean } | any>({value: 0, error: false});
    const [category, setCategory] = useState<{value: string, error: boolean } | any>({value: '', error: false});
        ...
        if (inputCheck()) {
          ...
          UPDATE_PRODUCT(id, data)
            .then((res: AxiosResponse<ApiResponse>) => {
                ...
                dispatch(UPDATE_PRODUCT_STATE(payload));
            })
            .catch((err: any) => {
    ...
    useEffect(() => {
        setProductImage({value: product?.productImage, error: false});
        setName({value: product?.name, error: false});
        setSku({value: product?.sku, error: false});
        setMake({value: product?.make, error: false});
        setDescription({value: product?.description, error: false});
        setCategory({value: product?.category?.id, error: false});
        setPrice({value: product?.price, error: false});
        setDiscount({value: product?.discount, error: false});
        setQuantity({value: product?.quantity, error: false});
    }, [])
