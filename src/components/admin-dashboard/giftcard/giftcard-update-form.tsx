...
import { UPDATE_GIFTCARD } from '../../../services';
import { UPDATE_GIFTCARD_STATE } from '../../../store/giftcards';
...
const GiftcardUpdateForm = ({ giftcard }: Props) => {
    ...
    const handleSubmit = () => {    
        if (inputCheck()) {
            ...
            const data = { 
                ...
                rate: rate.value,
                type: type.value,
                currencies: currencies.value.map(item => ({name: item.name, rate: item.rate}))
            };
    ...
    useEffect(() => {
        ...
        setCurrencies({value: giftcard ? giftcard.currencies : [], error: false});
    }, [])
